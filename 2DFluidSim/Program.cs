...
class Program
{
    ...
    static void Main()
    {
        ...
        Vector3[] pillarVertices = new Vector3[]
        {
            ...
            new Vector3(pillar.MaxX, pillar.MinY, 0),
            new Vector3(pillar.MaxX, pillar.MaxY, 0),
            new Vector3(pillar.MinX, pillar.MaxY, 0),
            new Vector3(pillar.MinX, pillar.MinY, 0)
        };
        
        // --- Objects ---
        // Fluid particles spawned dynamically in the upper right corner of the box
        List<FluidParticle> particles = new List<FluidParticle>();
        Random random = new Random();
        
        float spawnMinX = 0.0f; 
        float spawnMaxX = box.MaxX - 0.1f;
        float spawnMinY = 0.0f;
        float spawnMaxY = box.MaxY - 0.1f;

        for (int i = 0; i < particleAmount; i++)
        {
            float x = spawnMinX + (float)random.NextDouble() * (spawnMaxX - spawnMinX);
            float y = spawnMinY + (float)random.NextDouble() * (spawnMaxY - spawnMinY);
            FluidParticle particle = new FluidParticle(new Vector3(x, y, 0f), 0.025f);
        ...
        int pillarVao = GL.GenVertexArray();

        int particleVbo = GL.GenBuffer();
        int boundVbo = GL.GenBuffer();
        int pillarVbo = GL.GenBuffer();

        uint particlePosition = (uint)GL.GetAttribLocation(particleShader.Id, "vPosition");
        ...
        GL.EnableVertexAttribArray(boundPosition);

        // Pillar
        GL.BindVertexArray(pillarVao);
        GL.BindBuffer(BufferTarget.ArrayBuffer, pillarVbo);
        GL.BufferData(BufferTarget.ArrayBuffer, pillarVertices.Length * Vector3.SizeInBytes, pillarVertices, BufferUsage.StaticDraw);
        GL.VertexAttribPointer(boundPosition, 3, VertexAttribPointerType.Float, false, sizeof(float) * 3, 0);
        GL.EnableVertexAttribArray(boundPosition);
        ...
        while (true)
        {
            ...
            foreach (var particle in particles) 
            {
                // Updated to pass both the outer container and the internal pillar obstacle
                particle.UpdatePosition(box, pillar, particles, dt);
            ...
            // --- Rendering Bounding Box and Pillar ---
            boundShader.Use();
            ...
            // Draw Pillar
            GL.BindVertexArray(pillarVao);
            GL.DrawArrays(PrimitiveType.LineStrip, 0, pillarVertices.Length);
    ...
    public static float ConvertDensityToPressure(float density)
    {
        ...
        return float.Max(0, densityError) * pressureMultiplier;
    }
